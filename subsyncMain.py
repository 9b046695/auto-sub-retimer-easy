...
def fix_styling(directoryPath):
    extracted_subs = [f for f in os.listdir(directoryPath) if '.EXTRACTED.' in f]
...
def retime(mkv, srt, conf, directoryPath):
    retimed = mkv.replace(".mkv", ".ja.RETIMED.{}".format(conf['ext']))
    extracted = [f for f in os.listdir(directoryPath) if mkv.replace(".mkv", ".EXTRACTED.") in f][0]
...
if __name__ == '__main__':
    ...
    directoryPath = input('Please enter the absolute directory path that contains the episodes and the subtitles: ')
