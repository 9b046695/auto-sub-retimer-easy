...
if __name__ == '__main__':
    ...
    files = os.listdir()
    CONF = {}
    srts = sorted([f for f in files if f.endswith(f".srt")], key=alphanum_key)
    asss = sorted([f for f in files if f.endswith(f".ass")], key=alphanum_key)
    choice = 1
    if len(srts) == 0 and len(asss) != 0:
        choice = 2
    if len(srts) != 0 and len(asss) == 0:
        choice = 1
    if len(srts) != 0 and len(asss) != 0:
    ...
    if "2" in retime_choice:
        ...
        index = None
        for mkv in mkvs:
            index = extract_eng_subs(os.path.abspath(mkv), index)
